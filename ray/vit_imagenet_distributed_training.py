    ...
    for epoch in range(start_epoch, config["epochs"]):
        ...
        for batch in train_dataloader:
            ...
            if world_rank == 0 and num_batches % 100 == 0:
                ...
                logger.log(f"  Epoch {epoch} [{num_batches}] "
        ...
        if world_rank == 0:
            val_str = (f"val_top1={val_top1:.2f}% val_top5={val_top5:.2f}%"
                       if val_top1 > 0 else "")
            logger.log(f"Epoch {epoch}: train_top1={train_top1:.2f}% "
                       f"loss={train_loss:.4f} {val_str} "
                       f"lr={lr:.6f} {img_sec:.0f} img/s")

    if world_rank == 0:
        logger.log(f"Training complete. Best top-1 accuracy: {best_top1:.2f}%")
    logger.close()
...
def main():
    ...
    log_path  = make_log_path(f"vit_{args.model_size}_imagenet")
