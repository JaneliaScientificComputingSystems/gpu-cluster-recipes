...
def train_func(config):
    ...
    for it in range(start_iter, config["max_iters"]):
        ...
        if it % 50 == 0 and world_rank == 0:
            ...
            logger.log(f"iter {it:5d} | loss {accum_loss:.4f} | "
        ...
        if it % config["eval_interval"] == 0 and it > 0:
            ...
            if world_rank == 0:
                ...
                logger.log(f"  val_loss {val_loss:.4f} | {tok_sec/1e3:.1f}K tok/s")
    ...
    if world_rank == 0:
        ...
        logger.log(f"Done. {tokens_total/1e9:.2f}B tokens | "
...
def main():
    ...
    log_path  = make_log_path(f"gpt2_{args.model_size}_{args.mode}")
