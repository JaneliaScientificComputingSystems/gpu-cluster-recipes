...
from train_logging import make_log_path, MetricLogger
    ...
    for epoch in range(start_epoch, config["epochs"]):
        ...
        for batch_idx, (data, targets) in enumerate(trainloader):
            ...
            if batch_idx % 50 == 0 and world_rank == 0:
                logger.log(f"  Epoch {epoch} [{batch_idx}/{len(trainloader)}] "
                           f"loss={loss.item():.4f}")
        ...
        if world_rank == 0:
            logger.log(f"Epoch {epoch}: train={train_acc:.2f}%  test={test_acc:.2f}%")

    if world_rank == 0:
        logger.log(f"Training complete. Best test accuracy: {best_test_acc:.2f}%")
    logger.close()
...
def main():
    ...
    log_path       = make_log_path("cifar10_resnet18")
