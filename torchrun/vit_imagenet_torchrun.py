...
class ImageNetParquetDataset(Dataset):
    """Reads ImageNet from HuggingFace parquet shards (image bytes + label).

    Loads only the shards it is handed into a compact Arrow table and decodes
    rows lazily in __getitem__. Callers give each DDP rank its own shard subset,
    so a rank holds ~1/world_size of the data in RAM instead of every rank
    materializing the whole dataset — the latter (an eager `.to_pylist()` of all
    ~1.2M images per process) is what wedged the old version at startup past the
    NCCL rendezvous timeout. See this example's README.
    """
    ...
    def __init__(self, parquet_files, transform=None):
        ...
        if not parquet_files:
            raise ValueError("ImageNetParquetDataset received no shards for this rank")
        self.transform = transform
        tables = [pq.read_table(f, columns=["image", "label"]) for f in parquet_files]
        table = pa.concat_tables(tables) if len(tables) > 1 else tables[0]
        # keep columnar Arrow arrays; decode individual rows on demand (no to_pylist)
        self.images = table.column("image")
        self.labels = table.column("label")

    def __len__(self):
        return len(self.labels)

    def __getitem__(self, idx):
        img = Image.open(io.BytesIO(self.images[idx].as_py()["bytes"])).convert("RGB")
        if self.transform:
            img = self.transform(img)
        return img, self.labels[idx].as_py()
...
def main():
    ...
    # Give each rank its own subset of TRAIN shards so no rank materializes the
    # whole dataset. Each rank shuffles within its own shards (DataLoader below) —
    # standard DDP data parallelism, not a global shuffle. VAL keeps the full shard
    # set + a DistributedSampler: it is small, and equal-length disjoint coverage
    # is needed for a correct all-reduced accuracy.
    train_files_rank = train_files[world_rank::world_size]
    train_dataset = ImageNetParquetDataset(train_files_rank, transform=train_transform)
    val_dataset = ImageNetParquetDataset(val_files, transform=val_transform)

    if world_rank == 0:
        print(f"Dataset loaded: {len(train_files_rank)} train shards/rank "
              f"({len(train_dataset)} imgs/rank), {len(val_dataset)} val imgs "
    ...
    # Training loop
    # -----------------------------------------------------------------------
    # Ranks hold slightly different shard counts → different #batches/epoch. Cap
    # every rank to the global minimum, or a rank that runs short would desync
    # DDP's gradient all-reduce and hang the whole job.
    _steps = torch.tensor([len(train_loader)], device=device)
    dist.all_reduce(_steps, op=dist.ReduceOp.MIN)
    steps_per_epoch = int(_steps.item())
    if world_rank == 0:
        print(f"Steps/epoch (min across ranks): {steps_per_epoch}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (data, targets) in enumerate(train_loader):
            if batch_idx >= steps_per_epoch:   # keep all ranks lock-step for DDP
                break
            ...
            if world_rank == 0 and (batch_idx + 1) % 100 == 0:
                ...
                print(f"  Epoch {epoch} [{batch_idx + 1}/{steps_per_epoch}] "
