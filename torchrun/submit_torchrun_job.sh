...
# Sources ../nccl-ib/nccl_ib.sh automatically for InfiniBand queues.
#
# Usage:
...
usage() {
    ...
    echo "  --host=\"H1 H2 ...\"   Pin job to specific hosts (#BSUB -m); useful for node isolation"
    echo ""
...
case $QUEUE_NAME in
    gpu_b300_parallel|gpu_b300) NCCL_ENV_FILE="${SCRIPT_DIR}/../nccl-ib/nccl_ib_b300.sh" ;;
    *)                          NCCL_ENV_FILE="${SCRIPT_DIR}/../nccl-ib/nccl_ib.sh" ;;
esac
...
HOST_LINE=""
if [ -n "$HOST_SELECT" ]; then
    HOST_LINE="#BSUB -m \"${HOST_SELECT}\""
fi
